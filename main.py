...
async def on_message(message):
    ...
    if message.content.startswith(access_backup):
        backup_file = "```md\nCo_list : %s\nAt_info : %s\nEn_info : %s\nLv_info : %s\n```" % (Co_list, At_info, En_info, Lv_info)
        embed = discord.Embed(decryption=backup_file)
        global cha
        await cha.send(embed=embed)

    if message.author.id == "545506402383429642" and message.content.startswith("!정보"):  # !정보 (유저 아이디)
        msg = text_splitting(message.content)
        if msg[1] in At_info:
            at_info = At_info.get(msg[1])
        else:
            at_info = "NONE"
        if msg[1] in En_info:
            en_info = En_info.get(msg[1])
        else:
            en_info = "NONE"
        if msg[1] in Lv_info:
            lv_info = Lv_info.get(msg[1])
        else:
            lv_info = "NONE"
        info = "%s\n%s\n%s" % (at_info, en_info, lv_info)
        global cha
        await cha.send(info)
